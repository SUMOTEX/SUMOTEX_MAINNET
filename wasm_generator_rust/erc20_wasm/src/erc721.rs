...
pub struct ERC721Token {
    ...
    pub token_to_ipfs: HashMap<u64, String>,
...
impl ERC721Token {
    ...
    #[no_mangle]
    pub extern "C" fn mint(owner_ptr: *const u8, owner_len: usize, ipfs_hash_ptr: *const u8, ipfs_hash_len: usize) -> u32 {
        let token = unsafe {
    ...
    #[no_mangle]
    pub extern "C" fn read_token(&self, token_id: u64) -> Vec<u8> {
        if let Some(details) = self.get_token_details(token_id) {
            if let Ok(serialized_data) = bincode::serialize(&details) {
                return serialized_data;
            }
        }
        Vec::new()  // Return an empty vector on error or if token is not found.
    }
    fn get_token_details(&self, token_id: u64) -> Option<TokenDetails> {
