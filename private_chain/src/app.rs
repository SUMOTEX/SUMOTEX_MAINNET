...
impl App {
    ...
    pub fn get_blocks(&self)->Vec<block::Block>{
        self.blocks.clone()
    }
    ...
    pub fn try_add_block(&mut self, block: block::Block) {
        let latest_block = self.blocks.last().expect("there is at least one block");
        if block::Block::is_block_valid(&block, latest_block) {
            self.blocks.push(block);
        } else {
            error!("could not add block - invalid");
        }
    }
    pub fn is_chain_valid(&self, chain: &[block::Block]) -> bool {
        for i in 0..chain.len() {
            ...
            //let block_instance = block::Block::new();
            if !block::Block::is_block_valid(second, first) {
                return false;
            }
        }
        true
    ...
    pub fn choose_chain(&mut self, local: Vec<block::Block>, remote: Vec<block::Block>) -> Vec<block::Block> {
